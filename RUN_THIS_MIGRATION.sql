-- SAFE TO RUN - You have 0 users
-- This will clean up any partial migration and set up correctly

-- 1. Drop existing types (safe since no users)
DROP TYPE IF EXISTS user_type CASCADE;
DROP TYPE IF EXISTS account_status CASCADE;
DROP TYPE IF EXISTS verification_status CASCADE;
DROP TYPE IF EXISTS subscription_tier CASCADE;

-- 2. Drop tables if they partially exist
...
-- 3. Now run the full migration with the fix
-- Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- User types (with 'banned' included in verification_status)
...
-- Rest of migration continues below...CREATE TABLE public.users (
